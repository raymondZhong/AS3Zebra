package zebraMobileUI.util
{
	...
	public class Scale9GridBitmap extends Bitmap
	{
		...
		public function Scale9GridBitmap(bitmapData:BitmapData = null, scale9Grid:Rectangle = null, pixelSnapping:String = "auto", smoothing:Boolean = false)
		{
			...
			_sourceBmd = bitmapData.clone();
			_w = _sourceBmd.width;
			_h = _sourceBmd.height;
		...
		private function correctGridRect(rect:Rectangle):Rectangle
		{
			...
			if (rect.top < 1)
				rect.top = minMargin;
			if (rect.right > _sourceBmd.width - minMargin)
				rect.right = _sourceBmd.width - minMargin;
			if (rect.bottom > _sourceBmd.height - minMargin)
				rect.bottom = _sourceBmd.height - minMargin;
			return rect;
		}
		
		public function updateState(bitmapData:BitmapData, rect:Rectangle = null):void
		{
			_sourceBmd = bitmapData.clone();
			if (rect == null)
			{
		...
		private function build9GridBmds():void
		{
			...
			_tl.copyPixels(_sourceBmd, new Rectangle(0, 0, _scale9Grid.x, _scale9Grid.y), new Point());
			_tc = new BitmapData(_scale9Grid.width, _scale9Grid.y);
			_tc.copyPixels(_sourceBmd, new Rectangle(_scale9Grid.x, 0, _scale9Grid.width, _scale9Grid.y), new Point());
			_tr = new BitmapData(_sourceBmd.width - _scale9Grid.right, _scale9Grid.y);
			_tr.copyPixels(_sourceBmd, new Rectangle(_scale9Grid.right, 0, _sourceBmd.width - _scale9Grid.right, _scale9Grid.y), new Point());
			
			//中三格的位图数据
			_ml = new BitmapData(_scale9Grid.x, _scale9Grid.height);
			_ml.copyPixels(_sourceBmd, new Rectangle(0, _scale9Grid.y, _scale9Grid.x, _scale9Grid.height), new Point());
			_mc = new BitmapData(_scale9Grid.width, _scale9Grid.height);
			_mc.copyPixels(_sourceBmd, _scale9Grid, new Point());
			_mr = new BitmapData(_sourceBmd.width - _scale9Grid.right, _scale9Grid.height);
			_mr.copyPixels(_sourceBmd, new Rectangle(_scale9Grid.right, _scale9Grid.y, _sourceBmd.width - _scale9Grid.right, _scale9Grid.height), new Point());
			
			//底三格的位图数据
			_bl = new BitmapData(_scale9Grid.x, _sourceBmd.height - _scale9Grid.bottom);
			_bl.copyPixels(_sourceBmd, new Rectangle(0, _scale9Grid.bottom, _scale9Grid.x, _sourceBmd.height - _scale9Grid.bottom), new Point());
			_bc = new BitmapData(_scale9Grid.width, _sourceBmd.height - _scale9Grid.bottom);
			_bc.copyPixels(_sourceBmd, new Rectangle(_scale9Grid.x, _scale9Grid.bottom, _scale9Grid.width, _sourceBmd.height - _scale9Grid.bottom), new Point());
			_br = new BitmapData(_sourceBmd.width - _scale9Grid.right, _sourceBmd.height - _scale9Grid.bottom);
			_br.copyPixels(_sourceBmd, new Rectangle(_scale9Grid.right, _scale9Grid.bottom, _sourceBmd.width - _scale9Grid.right, _sourceBmd.height - _scale9Grid.bottom), new Point());
		}
